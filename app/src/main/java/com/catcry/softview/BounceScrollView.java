...
public class BounceScrollView extends ScrollView {
    ...
    protected void onFinishInflate() {
        if (getChildCount() > 0) {
            //获取的是ScrollView中的第一个子View，即LinearLayout
            inner = getChildAt(0);
        }
    }
    ...
    public void commOnTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_POINTER_UP:
                //当放下的第二根手指抬起时，继续计算位置，因为当你第二根手指抬起的时候，他会把滑动距离清零，用户体验不好
                isCount = false;
                break;
            case MotionEvent.ACTION_POINTER_DOWN:
                //当按下的第二根手指时，也让其计算位置，因为当你第二根手指放下时，他会把滑动距离清零，用户体验不好
                isCount = false;
                break;
            /***
