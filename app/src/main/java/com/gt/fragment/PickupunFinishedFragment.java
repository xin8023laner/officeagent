 ...
 */
public class PickupunFinishedFragment extends BaseFragment {
	...
	public void requestCallBack(int requestCode, MsMessage msm,
		...
		case Constant.querySendTaskList:
			...
			if (msm.getResult() == 0) {// 成功
				...
				getEventBus().post(new TabEvent(0, taskWaybills.size()));
				pickupAdapter = new PickupAdapter(getActivity(), taskWaybills,Constant.LIST_PICKUP_UNPICAUP);
...
//			getEventBus().post(new TabEvent(0, taskWaybills.size()));
	...
	public void onEvent(GetSearchTextPickupEvent event) {
		...
		if (event.getResult().equals("")) {
			...
			getEventBus().post(new ChangeTabPickupEvent(Constant.TAB_PICKUP_UNFINISHED));
		}
