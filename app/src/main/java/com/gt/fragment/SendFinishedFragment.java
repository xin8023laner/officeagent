 ...
 */
public class SendFinishedFragment extends BaseFragment {

	public static int list_type = Constant.LIST_SEND_SUCCESS;
...
//				showLoadingDialog(Constant.queryReceiveTaskList);
	...
	public void requestCallBack(int requestCode, MsMessage msm,
		...
		case Constant.queryReceiveTaskList:
			...
			if (msm.getResult() == 0) {// 成功
				...
				getEventBus().post(new SendTabEvent(1, agentReceiveTasks.size()));
				sendAdapter = new SendAdapter(getActivity(), agentReceiveTasks,Constant.LIST_SEND_SUCCESS);
	...
	public void onEvent(GetSearchTextEvent event) {
		if (event.getResult().equals("")) {
			...
			getEventBus().post(new ChangeTabEvent(Constant.TAB_SEND_FINISHED));
		}
	}
