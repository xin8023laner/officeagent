 ...
 * pda操作
 */
public class CapturePDAActivity extends BaseActivity implements OnKeyListener {
	private Dialog dialog, dialogRuKu;
	private int type, rukuCounts; // 扫描类型，分为单个扫描和多个扫描,待入库单数
	private ArrayList<String> results, qianshouResult, contacts, phones,
			paisongResult; // 多个扫描结果集
	...
	private List<AgentReceiveTask> listAgentReceiveTasks, qianshouList,
			paisongList;
	...
	private List<PickUpBySelf> pickUpBySelfList;
	...
	public int bindLayoutId() {
		return R.layout.activity_capturepda;
	}
			...
			if (type == Constant.SCAN_KEHUQIANSHOU) {
				...
				getTopbar().setOnControlOneListener(new OnControlOneListener() {
					...
					public void onControlOneListener() {
						// KeyBoardUtils.closeKeybordShowCursor(
						// CapturePDAActivity.this, viewUtils.view(
						// R.id.edt_scan_expresscode, EditText.class));
						// 看数据库里是否有未签收的
						long agentUserId = SPUtils.getUser(
								CapturePDAActivity.this).getAgentUserId();
				...
				List<AgentReceiveTask> list;
				list = getDbUtils().findAll(
						Selector.from(AgentReceiveTask.class).where("type",
								"=", type));
				if (list != null && list.size() > 0) {
...
//				viewUtils.setViewText(R.id.tv_scan_bottom,
//						"温馨提示：请扫描(可通过PDA扫描)快递单号联系客户，试试吧！");
...
//							viewUtils.setOnClickListener(R.id.iv_expressCodeclear);
				...
				viewUtils.setVisiable(R.id.ll_scan_paisong,
						R.id.tv_scan_ruku_count);
				viewUtils.setViewText(R.id.tv_scan_tip, "准备完毕，请开始扫描");
				viewUtils.setViewText(R.id.tv_scan_ruku_count, "待入库"
						+ rukuCounts + "单");
				viewUtils.view(R.id.tv_scan_tip, TextView.class).setTextColor(
						Color.BLACK);
				viewUtils.setViewText(R.id.btn_scan_expresscodeok, "添加");
				viewUtils.setOnClickListener(R.id.btn_scan_expresscodeok,
						R.id.ll_scan_person, R.id.btn_scan_fenpei);
				scanAdapter = new ScanAdapter(CapturePDAActivity.this,
				...
				viewUtils.view(R.id.edt_scan_expresscode, EditText.class)
						.requestFocus();
				viewUtils
						.setViewText(R.id.tv_scan_tip,
								"温馨提示：\n1.请扫描或输入要处理的滞留件单号或手机号后4位\n2.扫描后将选择或直接进入滞留件处理详情");
				viewUtils.view(R.id.edt_scan_expresscode, EditText.class)
						.setHint("输入快递单号或手机号后4位查找快件");
				viewUtils.setGone(R.id.btn_scan_result);
				viewUtils.setViewText(R.id.btn_scan_expresscodeok, "确定");
				getTopbar().setTitleText("联系客户");
				viewUtils.setOnClickListener(R.id.btn_scan_expresscodeok);
...
//				T.showShort(this, "快递单号不能为空！");
//				return;
//			} else if (viewUtils.getViewText(R.id.edt_scan_expresscode)
//					.length() < 6
			...
			if (type == Constant.SCAN_KEHUQIANSHOU_MORE) {// 批量签收
				...
				if (qianshouResult.contains(viewUtils
					...
					for (int i = 0; i < results.size(); i++) {
						if (viewUtils.getViewText(R.id.edt_scan_expresscode)
								.equals(results.get(i))) {
							T.showShort(CapturePDAActivity.this, "快递单号重复啦，换一个吧");
							return;
						}
					}
					if (dialog == null || !dialog.isShowing()) {
...
//			scanAdapter1.updateAll(list_contacts);
		...
		case R.id.ll_dialog_ruku_queren:
			if (agentReceiveTask != null && vDialogUtils != null) {
				agentReceiveTask.setReceiverPhone(vDialogUtils
						.getViewText(R.id.tv_dialog_rukutelephone_text));
				try {
				...
				if (type == Constant.SCAN_KUAIJIANRUKU) {
					viewUtils.view(R.id.lv_scan, ListView.class).setSelection(
							listAgentReceiveTasks.size() - 1);
					viewUtils.view(R.id.btn_scan_result, Button.class).setText(
							"批量入库(" + listAgentReceiveTasks.size() + ")");
				} else {
					viewUtils.view(R.id.lv_scan, ListView.class)
							.setSelection(0);
					viewUtils.setViewText(R.id.tv_scan_tip, "已扫描"
							+ listAgentReceiveTasks.size() + "单");
	...
	private void chooseAgentUser() {
		vDialogUtils = DialogUtils.showChooseAgentUserDialog(this,
				agentUserList, pickUpBySelfList,
				new ChooseAgentUserOrPickupBySelfListener() {
					@Override
					public void chooseAgentUserOrPickupBySelfResult(
							Object result) {
						if (result instanceof AgentUser) {
							selectUser = (AgentUser) result;
							viewUtils.setViewText(R.id.tv_scan_person,
									selectUser.getAgentUserName());
						} else {
							pickUpBySelf = (PickUpBySelf) result;
							viewUtils.setViewText(R.id.tv_scan_person,
									pickUpBySelf.getPickUpBySelfName());
						}
					}
				});
		final WheelView wv_agentUserOrPickupBySelf = vDialogUtils.view(
				R.id.wv_agentUser, WheelView.class);
		final AgentUserWheelAdapter agentUserAdapter = new AgentUserWheelAdapter(
				agentUserList);
		final PickupBySelfWheelAdapter pickupBySelfAdapter = new PickupBySelfWheelAdapter(
				pickUpBySelfList);
		...
		vDialogUtils.view(R.id.rg_dialog_uorpick, RadioGroup.class)
	...
	public void doRequest(int requestCode, Object... needParams) {
		...
		case Constant.queryAgentUsers:// 查询代办下所有代办员
			...
			requestServer(Constant.RECEIVETASK_SERVER,
					Constant.queryAgentUsers, AppUtils.createParams(
							ApiCode.queryAgentUsers.code, paramsValues11));
			break;
		case Constant.editReceiveTaskPaiSongList:
			...
			requestServer(Constant.RECEIVETASK_SERVER,
					Constant.editReceiveTaskPaiSongList, AppUtils.createParams(
		...
		case Constant.queryReceiveTask:
			try {
				if (msm.getResult() == Constant.ERROR_ONTINLIB) {// 未入库
					switch (type) {
					// case Constant.SCAN_NORMOL:// 录入信息
...
//							}
						...
						viewUtils.view(R.id.tv_scan_tip, TextView.class)
								.setText(
										Html.fromHtml("已扫描    "
												...
												+ listAgentReceiveTasks.size()
												+ "</font>" + "    单"));
						viewUtils.view(R.id.tv_scan_ruku_count, TextView.class)
								.setText(
										Html.fromHtml("待入库    "
												+ "<font color='red'>"
												+ rukuCounts + "</font>"
		...
		case Constant.editReceiveTaskQianshouList:
			if (msm.getResult() == 0) {// 成功
				long agentUserId = SPUtils.getUser(CapturePDAActivity.this)
						.getAgentUserId();
				List<AgentReceiveTask> list;
				try {
					list = getDbUtils().findAll(
				...
				DialogUtils
						...
						.setOnClickListener(new OnClickListener() {
							...
							public void onClick(View arg0) {
								...
								CapturePDAActivity.this.finish();
							}
						});
			}
			T.showShort(this, msm.getMessage());
		...
		case Constant.queryReceiveTaskPaiSongList:
			try {
				...
				if (msm.getResult() == 0) {
					...
					paisongList = new Gson().fromJson(msm.getData().toString(),
							new TypeToken<List<AgentReceiveTask>>() {
							}.getType());
		...
		case Constant.queryAgentUsers:// 查询代办点下所有代办员
			if (msm.getResult() == 0) {
				try {
					...
					pickUpBySelfList = new Gson().fromJson(
							arrayZiTi.toString(),
							new TypeToken<List<PickUpBySelf>>() {
							}.getType());
					agentUserList = new Gson().fromJson(
							arrayAgentUser.toString(),
							new TypeToken<List<AgentUser>>() {
							}.getType());
					if((agentUserList!=null&&agentUserList.size()>0)||(pickUpBySelfList!=null&&pickUpBySelfList.size()>0)){
						chooseAgentUser();
					} else {
		...
		case Constant.editReceiveTaskPaiSongList:
			if (msm.getResult() == 0) {// 成功
				...
				vDialogUtils = DialogUtils.showTipDialog(this, 0, "分配成功！",
						Constant.SCAN_PAISONG);
				...
				vDialogUtils.view(R.id.btn_dialog_paisong).setOnClickListener(
						new OnClickListener() {
...
//								@Override
//								public void onClick(View v) {
//									doRequest(Constant.queryCustomer);
//									DialogUtils.cancelDialog();
//								}
//							});
				}
...
//				}
	...
	private void editPQDialog(final List<AgentReceiveTask> list) {
		...
		vDialogUtils = showRUKUDialog(this,
				"当前有未完成的" + String.valueOf(list.size()) + str);
				...
				new OnClickListener() {
					...
					public void onClick(View v) {
						...
						if (type == Constant.SCAN_KEHUQIANSHOU) {
							...
							viewUtils.view(R.id.lv_scan, ListView.class)
									.setAdapter(scanAdapter);
							type = Constant.SCAN_KEHUQIANSHOU_MORE;
							doRequest(Constant.queryReceiveTaskQianshouList);
						} else {
							paisongResult = new ArrayList<String>();
							paisongList = new ArrayList<AgentReceiveTask>();
...
//										.getViewText(R.id.edt_scan_expresscode));
								...
								artContact.setAgentId(SPUtils.getAgent(
										CapturePDAActivity.this).getAgentId());
								artContact.setStype("1");
								artContact.setAgentUserCode(SPUtils.getUser(
										CapturePDAActivity.this)
										.getAgentUserCode());
								artContact.setAgentUserId(SPUtils.getUser(
										CapturePDAActivity.this)
										.getAgentUserId());
								artContact.setAgentUserName(SPUtils.getUser(
										CapturePDAActivity.this)
										.getAgentUserName());
								artContact.setScanTime(System
										.currentTimeMillis());
								artContact.setReceiverPhone(vDialogUtils
										.getViewText(R.id.tv_dialog_rukutelephone_text));
							...
							T.showShort(CapturePDAActivity.this, "请选择快递公司！");
						}
	...
	public void onEvent(ScanResultEvent event) {
		...
		case ScanResultEvent.TYPE_SCANBULU:// 补录/更改手机号
			...
			KeyBoardUtils.closeKeybordShowCursor(this, vDialogUtils.view(
					R.id.tv_dialog_rukutelephone_text, EditText.class));
			vDialogUtils.view(R.id.ll_dialog_ruku_queren, Button.class)
					.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View v) {
							if (TextUtils.isEmpty(vDialogUtils
									.getViewText(R.id.tv_dialog_rukutelephone_text))) {
								T.showShort(CapturePDAActivity.this, "请输入手机号！");
							} else {
								for (int i = 0; i < listAgentReceiveTasks
										.size(); i++) {
									if (agentReceiveTask.getExpressCode()
											.equals(listAgentReceiveTasks
													.get(i).getExpressCode())) {
										listAgentReceiveTasks
												.get(i)
												.setReceiverPhone(
														vDialogUtils
																.getViewText(R.id.tv_dialog_rukutelephone_text));
										scanAdapter
												.updateAll(listAgentReceiveTasks);
										try {
											AgentReceiveTask task = getDbUtils()
													.findFirst(
															Selector.from(
																	AgentReceiveTask.class)
																	.where("expressCode",
																			...
																			agentReceiveTask
																					.getExpressCode()));
											if (task != null) {
												task.setReceiverPhone(vDialogUtils
														.getViewText(R.id.tv_dialog_rukutelephone_text));
									...
									.setText(
											Html.fromHtml("已扫描    "
													...
													+ listAgentReceiveTasks
															.size() + "</font>"
													+ "    单"));
							viewUtils.view(R.id.tv_scan_ruku_count,
									TextView.class).setText(
									Html.fromHtml("待入库    "
											+ "<font color='red'>" + rukuCounts
											+ "</font>" + "    单"));
	...
	public void onEvent(UpdateZhiLiuList event) {
		if (!TextUtils
				.isEmpty(viewUtils.getViewText(R.id.edt_scan_expresscode))
				&& listAgentReceiveTasks.size() > 1) {
	...
	protected void onScan(String barcode) {
		...
		if (barcode != null) {
			mSoundUtils.init(this);
			if (barcode.length() < 6 || barcode.contains("http")) {
				mSoundUtils.warn();
				T.showShort(this, "您的单号格式不正确！");
				return;
			} else {
			...
			case Constant.SCAN_KUAIJIANRUKU:
				viewUtils.setViewText(R.id.edt_scan_expresscode, barcode);
				if("韵达快递".equals(dbAgentCarrier.getParentCarrierName())
						&&barcode.length()>13){
					viewUtils.setViewText(R.id.edt_scan_expresscode, TextUtils.substring(barcode, 0, 13));
				}
				for (int i = 0; i < results.size(); i++) {
					if (viewUtils.getViewText(R.id.edt_scan_expresscode).equals(results.get(i))) {
						T.showShort(CapturePDAActivity.this, "快递单号重复啦，换一个吧");
			...
			case Constant.SCAN_KEHUZITI:
				result(1, new IntentExtra(1, barcode));
				finish();
				break;
			case Constant.SCAN_CONTACT:
				viewUtils.setViewText(R.id.edt_scan_expresscode, barcode);
			...
			case Constant.SCAN_NORMAL:
				result(1, new IntentExtra(1, barcode));// pickup
				finish();
				break;
			case Constant.SCAN_KEHUQIANSHOU_MORE:// 批量签收
				...
				viewUtils.setViewText(R.id.edt_scan_expresscode, barcode);
				for(AgentReceiveTask art:qianshouList){
					if(barcode.contains(art.getExpressCode())&&"韵达快递".equals(art.getCarrierName())
							&&barcode.length()>13){
						viewUtils.setViewText(R.id.edt_scan_expresscode, TextUtils.substring(barcode, 0, 13));
						break;
					}
			...
			case Constant.SCAN_ZHILIUJIANCHULI:
				// if (viewUtils.view(R.id.edt_scan_expresscode, EditText.class)
				// .getHint().toString().equals("输入快递单号或手机号后4位查找快件")) {
				viewUtils.setViewText(R.id.edt_scan_expresscode, barcode);
				doRequest(Constant.zljcx);
				// }
				break;
	...
	private void saveAgentReceiveTaskPaisong() {
		if (agentReceiveTask != null) {
			if (!viewUtils.getViewText(R.id.edt_scan_expresscode).equals(
					agentReceiveTask.getExpressCode())) {
				agentReceiveTask.setReceiverPhone(vDialogUtils
						.getViewText(R.id.tv_dialog_rukutelephone_text));
				try {
				...
				results.add(agentReceiveTask.getExpressCode());
				// viewUtils.setViewText(R.id.tv_scan_tip, "任务列表（"
				// + listAgentReceiveTasks.size() + "/200）");
				viewUtils.view(R.id.tv_scan_tip, TextView.class).setText(
						Html.fromHtml("已扫描    " + "<font color='red'>"
								+ listAgentReceiveTasks.size() + "</font>"
								+ "    单"));
				viewUtils.view(R.id.tv_scan_ruku_count, TextView.class)
						.setText(
								Html.fromHtml("待入库    " + "<font color='red'>"
										+ rukuCounts + "</font>" + "    单"));
				viewUtils.setViewText(R.id.btn_scan_fenpei, "分配("
						+ listAgentReceiveTasks.size() + ")");
				agentReceiveTask = null;
	...
	private void saveAgentReceiveTasks(List<AgentReceiveTask> list,String telPhone) {
		if (type == Constant.SCAN_KUAIJIANRUKU) {// 入库
			...
			agentReceiveTask.setAgentUserName(SPUtils.getUser(this)
					.getAgentUserName());
			agentReceiveTask.setAgentUserCode(SPUtils.getUser(this)
					.getAgentUserCode());
			agentReceiveTask.setStype("2");
			agentReceiveTask.setType(type);
			agentReceiveTask.setExpressCode(viewUtils
					.getViewText(R.id.edt_scan_expresscode));
			...
			if (agentCarrier != null) {
				...
				results.add(agentReceiveTask.getExpressCode());
				// viewUtils.setViewText(R.id.tv_scan_tip,
				// "已扫描"+listAgentReceiveTasks.size()+"单");
				// viewUtils.setViewText(R.id.tv_scan_ruku_count,
				// "待入库"+rukuCounts+"单");
				viewUtils.view(R.id.tv_scan_tip, TextView.class).setText(
						Html.fromHtml("已扫描    " + "<font color='red'>"
								+ listAgentReceiveTasks.size() + "</font>"
				...
				if (TextUtils.isEmpty(agentReceiveTask.getReceiverPhone())) {
					vDialogUtils = showTelephoneDialog(this, agentReceiveTask);
					KeyBoardUtils.closeKeybordShowCursor(this, vDialogUtils
	...
	public ViewUtils showEditCallDialog(Context context, int type) {
		...
		ViewUtils viewUtils = new ViewUtils(context,
				R.layout.dialog_editcall_contacts);
	...
	public ViewUtils showInLibDialog(Context context) {
		...
		if (type == Constant.SCAN_PAISONG || type == Constant.SCAN_CONTACT
				|| type == Constant.SCAN_ZHILIUJIANCHULI) {
	...
	private void initPopWindow() {
				...
				.setOnItemClickListener(new OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> arg0, View arg1,
						...
						vDialogUtils.setViewText(
								R.id.tv_dialog_unfinishedReasen, list.get(arg2));
						excepReason = vDialogUtils
								.getViewText(R.id.tv_dialog_unfinishedReasen);
						popupWindow.dismiss();
					}
				});
		...
		popupWindow.showAsDropDown(vDialogUtils.view(R.id.tv_dialog_title), 0,
				0);
	}
	 ...
	 * @param requesxxxtCode
	 */
	public ViewUtils showTelephoneDialog(Context context,
		...
		dialog.setOnKeyListener(this);
		dialog.setContentView(viewUtils.getRootView());
		viewUtils.setViewText(R.id.tv_rukutelephone_danhao, "单号："
				+ agentReceiveTask.getExpressCode());
		viewUtils.setViewText(R.id.tv_dialog_rukutelephone_text,
				agentReceiveTask.getReceiverPhone());
	...
	public ViewUtils showEditCallDialog(Context context, String text) {
		...
		KeyBoardUtils.closeKeybordShowCursor(this, viewUtils.view(R.id.edt_dialog, EditText.class));
		dialog.setOnKeyListener(this);
		dialog.show();
		return viewUtils;
	}
					...
					public void onClick(View v) {
						if (type == ChangeContactPhoneEvent.TYPE_CONSTANTS_EXCEP) {
							if (TextUtils.isEmpty(excepReason)) {
								T.showShort(CapturePDAActivity.this, "请输入原因");
							} else {
							...
							if (TextUtils.isEmpty(vDialogUtils
									.getViewText(R.id.edt_dialog))) {
								T.showShort(CapturePDAActivity.this, "请输入联系方式");
							} else {
								// 并请求服务器，将其加入到列表中
								if (telephonyManager
										.getSimState() == TelephonyManager.SIM_STATE_ABSENT) {
								...
								// 拨打电话
								CommonUtils.call(CapturePDAActivity.this,
										agentContactCustomer.getReceiverPhone());
								}
								dialog.cancel();
							}
		...
		if (event.getKeyCode() == KeyEvent.KEYCODE_DPAD_CENTER) {
			...
			case Constant.SCAN_NORMAL:
				if (viewUtils.getViewText(R.id.edt_scan_expresscode).equals("")) {
					...
					result(1,
							new IntentExtra(1, viewUtils
									.getViewText(R.id.edt_scan_expresscode)));// pickup
					finish();
				}
				break;
			case Constant.SCAN_KEHUQIANSHOU_MORE:
				if (listAgentReceiveTasks.size() > 0) {
					if (viewUtils.getViewText(R.id.btn_scan_result).equals(
							"确认签收")) {
						doRequest(Constant.editReceiveTaskQianshouList);
					} else {// 入库
						intent(InLibPreviewActivity.class, null);
	...
	public boolean onKey(DialogInterface dialog, int keyCode, KeyEvent event) {
		if (event.getAction() == KeyEvent.ACTION_DOWN) {
			...
			case KeyEvent.KEYCODE_BACK:
				if (type != Constant.SCAN_KUAIJIANRUKU && dialogRuKu != null
						&& dialogRuKu.isShowing()) {
					return true;
				} else if (dialog != null) {
					DialogUtils.cancelDialog();
					finish();
