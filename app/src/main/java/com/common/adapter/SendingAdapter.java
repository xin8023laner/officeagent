...
public class SendingAdapter extends BasicAdapter<AgentReceiveTask> {

	private int type = Constant.LIST_SEND_FAILED;
	private TelephonyManager telephonyManager;
	public SendingAdapter(Context context, List<AgentReceiveTask> data, int type) {
		super(context, data);
		this.type = type;
			...
			public void onClick(View arg0) {
				...
				CommonUtils.call(
						...
						TextUtils.isEmpty(agentReceiveTask.getReceiverPhone()) ? agentReceiveTask
								.getReceiverPhone() : agentReceiveTask
								.getReceiverPhone());
//				((MainActivity)getContext()).getEventBus().post(new CallEvent(CallEvent.Send,null,agentReceiveTask));
				}
			}
		});
		return convertView;
