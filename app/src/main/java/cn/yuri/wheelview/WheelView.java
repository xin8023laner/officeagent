...
public class WheelView extends View {
	...
	/** 备份颜色：0xFFFF0000 */
	private static final int VALUE_TEXT_COLOR = 0xFF252525;

	/** Items text color */
	private static final int ITEMS_TEXT_COLOR = 0xFF252525;
	...
	/** Text size */
	public int TEXT_SIZE = 32;

	/** Top and bottom items offset (to hide that) */
	private final int ITEM_OFFSET = TEXT_SIZE / 99;

	/** Additional width for items layout */
	private static final int ADDITIONAL_ITEMS_SPACE = 3;
	...
	private void initData(Context context) {
		...
		juli = (int) DensityUtils.px2dp(context, 14);
	}
	...
	private String buildText(boolean useCurrentValue, int widthItems) {
		...
		for (int i = currentItem - addItems; i <= currentItem + addItems; i++) {
			if (useCurrentValue || i != currentItem) {
				...
				text = measureTitle(text, widthItems, false);
				if (text != null) {
	...
	public String measureTitle(String title, int widthItems,
		...
		if (title != null) {
			...
			if (textPaint.measureText(title, 0, title.length()) < widthItems)
				return title;
	...
	private void createLayouts(int widthItems, int widthLabel) {
		if (itemsLayout == null || itemsLayout.getWidth() > widthItems) {
			itemsLayout = new StaticLayout(buildText(isScrollingPerformed,
					widthItems), itemsPaint, widthItems,
		...
		if (!isScrollingPerformed
			...
			String text = getAdapter() != null ? getAdapter().getItem(
					currentItem) : null;
			text = measureTitle(text, widthItems, true);
